...
import { CRAGS } from './crags.js?v=44';
...
// (1) it's ~0.15ms per call, which matters when scoreDay() runs hundreds of
// times per request, and (2) en-AU/en-GB CLDR abbreviates September as
// "Sept", which silently broke every September climate-normal lookup.
...
// Hoisted to module scope (v60 perf fix) — see worker/src/lib/forecast.js for
// the full rationale. Constructing a new Intl.DateTimeFormat on every call
// was ~14x slower than reusing one instance, and this runs on every hourly
// timestep inside computeSunWindow()/computeSolarExposure().
...
export function scoreDay(crag, day, prevDay, nextDay) {
  ...
  //
  // Perf + correctness note (v60): this used to be
  //   new Date(...).toLocaleString('en-AU', { month: 'short' })
  // which is (a) slow — toLocaleString/Intl formatting runs ~0.15ms per call,
  // which adds up fast across hundreds of scoreDay() calls per request, and
  // (b) actually wrong: the en-AU/en-GB CLDR locale abbreviates September as
  // "Sept" (4 letters), not "Sep", so it never matched the "Sep" key in
  // CLIMATE_PROFILES — every September climate-normal lookup silently
  // returned null. A plain lookup table is both faster and correct.
